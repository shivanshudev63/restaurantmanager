...
const CustomerRestaurantMenuPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await RestaurantFinder.get(`/customerhome/${id}/menu`);
  ...
  const handleclick=(id)=>{
    history(`/restaurants/customerhome/${id}/review`);
    ...
    <div>{selectedRestaurant && (
      ...
      <Box
        ...
        <div className='mt-3'>
          <Menus menus={selectedRestaurant.menus} />
        </div>
        <Footer />
      </>
    )}</div>
...
export default CustomerRestaurantMenuPage